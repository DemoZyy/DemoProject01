...
//        SENDER_ID = "506053237730";
        ...
        builder.setPositiveButton("Publish",
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        try {
                            pubnub.publish(channel, getJsonArray(message), publishCallback);
                            return;
                        } catch (Exception e) {
                        }

                        try {
                            pubnub.publish(channel, getJsonObject(message), publishCallback);
                            return;
                        } catch (Exception e) {
                        }
    ...
    private JsonObject getJsonObject(String text) {
        ...
        return (JsonObject)jsp.parse(text);
    }
    private JsonArray getJsonArray(String text) {
        JsonParser jsp = new JsonParser();
        return (JsonArray)jsp.parse(text);
    }
