...
import com.pubnub.test.MainActivity;


public class PubnubAndroidJavaTest extends
		ActivityInstrumentationTestCase2<MainActivity> {

	private Pubnub pubnub;
	private Pubnub pubnub_enc;
	private final String channel = "hello_world";

	public PubnubAndroidJavaTest() {
		super(MainActivity.class);
	}

	public void setUp() {
		pubnub = new Pubnub("demo", "demo");
		pubnub_enc = new Pubnub("demo", "demo", "demo", "demo", false);
		pubnub_enc.setCacheBusting(false);
		pubnub_enc.setOrigin("pubsub");
	...
	public void testPublish() {
		...
		// assertTrue(false); // only for sure adequate test working.
	}
	...
	public void testPublishJSONArray() {
		...
		assertEquals(1, pbCb.getResult());
		if (sbCb.getResponse() != null && sbCb.getResponse() instanceof JSONArray) {
	...
	public void testPublishJSONObject() {
		...
		try {
			...
			assertEquals(1, pbCb.getResult());
			if (sbCb.getResponse() != null && sbCb.getResponse() instanceof JSONObject) {
				assertEquals(sendMessage.toString(), sbCb.getResponse().toString());
			} else {
				assertTrue(false);
			}
	...
	public void testPublishStringWithEncrypt() {
		String channel = "java-unittest-" + Math.random();
	...
	public void testPublishJSONArrayWithEncrypt() {
		String channel = "java-unittest-" + Math.random();
		final JSONArray sendMessage = new JSONArray().put(1).put("Test");
	...
	public void testPublishJSONObjectWithEncrypt() {
		String channel = "java-unittest-" + Math.random();
	...
	public void testPresence() {
		...
		try {
			...
			pubnub2.subscribe(channel, new SubscribeCallback(latch));
	...
	public void testConnectionRestoreMultipleChannels() {
		...
		try {
			pubnub.setResumeOnReconnect(true);
			pubnub.subscribe(new String[] { channel, "b" },
					new SubscribeCallback(latch) {
						public void connectCallback(String channel,
								Object message) {
							pubnub.unsubscribe(channel);
							pubnub.publish(channel, 10, new PublishCallback(
									latch) {
								public void successCallback(String channel,
										Object message) {
									try {
	...
	public void testConnectionRestoreFalse() {
		...
		try {
			...
			assertTrue(null == subscribeCb.getResponse() || !subscribeCb.getResponse().equals(Integer.valueOf(10)));
	...
	public void testConnectionRestoreFalseUnsubInSuccessCallback() {
		...
		try {
			...
			latch.await(10, TimeUnit.SECONDS);
			assertTrue(null == subscribeCb.getResponse() || !subscribeCb.getResponse().equals(Integer.valueOf(20)));
	...
	public void testSubscribeInMultipleThreads() {
		...
		class SubscribeThread implements Runnable {
			...
			public void run() {
				try {
					pubnub.subscribe(ch, new Callback() {
	...
	public void testUlsGrantRW() {
		...
		try {
			assertNotNull(ulscb.getResult());
			assertEquals("sub-c-a478dd2a-c33d-11e2-883f-02ee2ddab7fe", ulscb
					.getResult().getString("subscribe_key"));
	...
	public void testUlsAuditChannel() {
		...
		try {
			assertNotNull(ulscb.getResult());
			assertTrue(ulscb.getResult().getJSONObject("channels")
					.getJSONObject("hello-uls") != null);
		} catch (JSONException e) {
			assertTrue(false);
		}
	}
	...
	/*  @Test
	  public void testPublishString() { 
		  ...
		  SubscribeCallback sbCb = new SubscribeCallback(latch) {
			  ...
			  pubnub.subscribe(channel, sbCb);
			  latch.await(30, TimeUnit.SECONDS);
			  Assert.assertEquals(1, pbCb.getResult());
			  Assert.assertEquals(sendMessage, sbCb.getResponse());
		  } catch(Exception e) {
			  assertTrue(false);
		  }
	  }*/
