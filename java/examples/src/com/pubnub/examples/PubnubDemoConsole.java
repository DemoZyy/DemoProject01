...
public class PubnubDemoConsole {
	...
	public PubnubDemoConsole(String publish_key, String subscribe_key,
			String secret_key, String cipher_key) {
					...
					if (message instanceof JSONObject) {
						...
						try {
							notifyUser("TIMETOKEN: " + timetoken + ", "
									+ ((JSONObject) message).getString("data"));
							notifyUser("TIMETOKEN: " + timetoken + ", "
									+ ((JSONObject) message).getString("data2"));
							notifyUser("TIMETOKEN: " + timetoken + ", "
						...
						System.out.println("TIMETOKEN: " + timetoken + ", "
								+ "Message not a json object : " + message);
					}
		...
		while ((command = reader.nextInt()) != 9) {
			...
			case 12:
				pubnub.setResumeOnReconnect(pubnub.isResumeOnReconnect() ? false
						: true);
				notifyUser("RESUME ON RECONNECT : "
			...
			case 36: {
				...
				final PubnubSyncedObject jso = pubnub.createSyncObject(
						objectId, path);
	...
	private int getIntFromConsole(String message, boolean optional) {
		...
		} while ((input == null || input.length() == 0 || returnVal < -1)
				&& !optional);
	...
	private boolean getBooleanFromConsole(String message, boolean optional) {
		...
		do {
			...
			attempt_count++;
				...
				&& !optional);
		returnVal = (input.equalsIgnoreCase("y") || input
	...
	private void displayMenuOptions() {
		...
		notifyUser("ENTER 12 FOR Toggle Resume On Reconnect ( current: "
				+ pubnub.getResumeOnReconnect() + " )");
		notifyUser("ENTER 13 FOR Setting MAX Retries ( current: "
				+ pubnub.getMaxRetries() + " )");
		notifyUser("ENTER 14 FOR Setting Retry Interval ( current: "
				+ pubnub.getRetryInterval() + " milliseconds )");
		notifyUser("ENTER 15 FOR Setting Window Interval ( current: "
				+ pubnub.getWindowInterval() + " milliseconds )");
		notifyUser("ENTER 16 FOR Setting Subscribe Timeout ( current: "
				+ pubnub.getSubscribeTimeout() + " milliseconds )");
		notifyUser("ENTER 17 FOR Setting Non Subscribe Timeout ( current: "
				+ pubnub.getNonSubscribeTimeout() + " milliseconds )");
		notifyUser("ENTER 18 FOR Setting/Unsetting auth key ( current: "
				+ pubnub.getAuthKey() + " )");
		notifyUser("ENTER 19 FOR PAM grant");
		notifyUser("ENTER 20 FOR PAM revoke");
		notifyUser("ENTER 21 FOR PAM Audit");
		notifyUser("ENTER 22 FOR Setting Origin ( current: "
				+ pubnub.getOrigin() + " )");
		notifyUser("ENTER 23 FOR Setting Domain ( current: "
				+ pubnub.getDomain() + " )");
		notifyUser("ENTER 24 FOR Enabling Cache Busting  ( current: "
				+ pubnub.getCacheBusting() + " )");
		notifyUser("ENTER 25 FOR Disabling Cache Busting ( current: "
				+ pubnub.getCacheBusting() + " )");
		notifyUser("ENTER 26 FOR Setting UUID ( current: " + pubnub.getUUID()
				+ " )");
		notifyUser("ENTER 27 FOR Setting Presence Heartbeat ( current: "
				+ pubnub.getHeartbeat() + " )");
		notifyUser("ENTER 28 FOR Setting Presence Heartbeat Interval ( current: "
				+ pubnub.getHeartbeatInterval() + " )");
